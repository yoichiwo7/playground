...
class HtmlChartjsWriter:
    ...
    def write_df_to_html(self, df, writer, max_samples=None) -> str:
        ...
        def make_table_generator(parent_tuple, cols, df):
            df_stats = (
                df[parent_tuple]
                .describe(percentiles=[.25, .50, .75, .90, .99, .999])
                .round(2)
            )
        ...
        def make_chart_generator(parent_tuple, cols, df, chart_type):
            ...
            chart_json = self.__get_chart_json_dict(
                df_data, parent_tuple, cols, border_colors, bg_colors, chart_type
            )
            return json.dumps(chart_json)

        titles = [" : ".join(parent_tuple) for parent_tuple in parents_leaf_dict.keys()]
        table_gn = (
            make_table_generator(parent_tuple, cols, df)
            for parent_tuple, cols in parents_leaf_dict.items()
        )
        line_chart_gn = (
            make_chart_generator(parent_tuple, cols, df, "line")
            for parent_tuple, cols in parents_leaf_dict.items()
        )
        area_chart_gn = (
            make_chart_generator(parent_tuple, cols, df, "area")
            for parent_tuple, cols in parents_leaf_dict.items()
        )
    ...
    def __get_chart_json_dict(
        self, df_data, parent_tuple, cols, border_colors, bg_colors, t
    ):
...
class HtmlEchartsWriter:
    ...
    def write_df_to_html(self, df, writer, max_samples=None) -> str:
        ...
        chart_gn = (
            make_chart_generator(parent_tuple, cols, df)
            for parent_tuple, cols in parents_leaf_dict.items()
        )
        ...
        html = tpl.render({"titles": titles, "tables": table_gn, "charts": chart_gn})
        if writer:
            writer.write(html)
        return html
    ...
    def __get_chart_json_dict(self, df_data, parent_tuple, cols):
        d = {
            "title": {"text": " ".join(parent_tuple)},
            "tooltip": {},
            "legend": {"data": cols},
            "xAxis": {"data": df_data.index.tolist()},
            "yAxis": {},
            "series": [
                {
                    "name": col,
...
class XlsxWriter:
    ...
    def write_df_to_excel(
        ...
        df_sampled_data.to_excel(
            writer,
            sheet_name_sampled_data,
            float_format="%.2f",
            index=index,
            freeze_panes=(row_start_pos, FIXED_COL_POS),
        )
        self.__add_chart_sheet(
            writer,
            parents_leaf_dict,
            sheet_name_sampled_data,
            name,
            ...
            row_start_pos,
            row_end_pos,
            chart_each=chart_each,
        )

    def __add_chart_sheet(
        self,
        writer,
        parents_leaf_dict,
        sheet_name_data,
        ...
        for parents, leafs in parents_leaf_dict.items():
            ...
            for col_pos, leaf in enumerate(leafs, 1):
                ...
                sub_chart.set_title({"name": f'{"::".join(parents)} [{leaf}]'})
    ...
    def __set_nice_chart(self, chart, style_id=2):
        ...
        chart.set_chartarea({"border": {"color": "black"}})
